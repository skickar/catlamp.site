...
(function () {
  ...
  function renderPane() {
    ...
    if (s) {
      ...
      box.textContent = images.length
        ...
        : 'No community images yet: be the first (see "Submit an image" below). You can still flash your own .bin with FLASH CUSTOM .BIN.';
      $('c-flash').textContent = '[ CONNECT & FLASH ]';
  ...
  async function doFlash() {
    var s = sel();
    if (!s) { log('!! no image selected: use FLASH CUSTOM .BIN with your own file'); return; }
    if (flashing) return;
